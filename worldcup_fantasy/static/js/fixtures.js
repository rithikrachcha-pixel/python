...
function renderFixtures(fixtures, backedNation){
  ...
  if(!fixtures || !fixtures.length){
    el.innerHTML = '<p class="muted" style="padding:12px 0;">No fixtures yet.</p>';
    return;
  }
  ...
  // Backed nation matches
  const myMatches = backedNation
    ? fixtures.filter(f => f.home_team === backedNation || f.away_team === backedNation)
    : [];

  let html = '';

  // ── Backed nation fixtures first ──
  if(myMatches.length && backedNation){
    html += `<div class="fx-stage-title">🌟 ${flag(backedNation)} ${backedNation}'s Fixtures</div>`;
    myMatches.forEach(f => { html += fixtureHtml(f, backedNation, true); });
  ...
  if(played.length){
    ...
    recent.forEach(f => { html += fixtureHtml(f, backedNation, false); });
    html += '<div class="divider"></div>';
  }
  ...
  if(showDates.length){
    ...
    showDates.forEach(date => {
      html += `<div style="font-size:.73rem;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1.2px;margin:12px 0 6px;padding-left:2px;">${fmtDate(date)}</div>`;
      byDate[date].forEach(f => { html += fixtureHtml(f, backedNation, false); });
    });
  }

  el.innerHTML = html || '<p class="muted" style="padding:12px 0;">No fixtures available.</p>';
}
...
function fixtureHtml(f, backedNation, highlight){
  ...
  const dateStr = f.match_date && !f.played
    ? `<span style="font-size:.72rem;color:var(--text3);margin-left:auto;">${fmtDate(f.match_date)}</span>`
    : '';
